$(document).ready(function() {
	...
	$("#openBtn").click(function(){
		...
		if(email.match(/[\s\w\d]+@[\s\w\d]+/g)){
			if($("#openPass").val().length>=10){
				if(true || $("#openPass").val()==$("#openPassConfirm").val()){
					var email = $("#openEmail").val().toLowerCase();
					...
					$("#walletHistory").attr('href','http://api.coinawesome.com/address/'+keys.address);

					$("#walletQrCode").html("");
					var qrcode = new QRCode("walletQrCode");
					qrcode.makeCode("awesome:"+keys.address);
	...
	$("#walletLogout").click(function(){
		...
		$("#walletAddress").html("");
		$("#walletHistory").attr('href','http://api.coinawesome.com/address/');

		$("#walletQrCode").html("");
		var qrcode = new QRCode("walletQrCode");
		qrcode.makeCode("awesome:");
	...
	$("#walletConfirmSend").click(function(){
		...
		var urlAmount = Number($("#urlAmount").val())
		console.log(urlShare, urlAmount)
		// var devaddr = coinjs.developer;
		// var devamount = 0//$("#developerDonation");

		// if((devamount.val()*1)>0){
		// 	tx.addoutput(devaddr, devamount.val()*1);
		// }

		var total = (Number(txfee.val())*1)
		console.log('fee is', total)
		// var total = (devamount.val()*1) + (txfee.val()*1);

		// if URL to be shared
		if(false && urlShare.substring(0,4) === 'http') {
			// var urlAddr = getAddrForUrl(cleanUrl(urlShare))

			console.log('addr for url is:', urlAddr)
			// if(!urlAmount)
			// 	var urlAmount = 100000000
			// total += urlAmount
			// tx.addoutput(urlAddr, urlAmount);


			// var data = {u: urlShare, t:[]}
			console.log(data)
			...
			$.each($("#walletSpendTo .output"), function(i,o){
				var addrOrUrl = $('.addressTo',o).val();
				var addr = '', url = '', data;

				if(addrOrUrl.substring(0,4) === 'http'){
					url = cleanUrl(addrOrUrl)
					addr = getAddrForUrl(url)

					console.log('addr for url is:', addr)
				...
				var amount = (Number($('#amountInput').val())*1);
				// var amount = (Number($('.amount',o).val())*1);
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			// console.log('dvalue', data.value/100000000)
			if(dvalue>=total){
				...
				tx2.broadcast(function(data){
					...
					if(data.txid){
						...
						$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-danger').html(JSON.stringify(data));
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+data.value+" AWE unable to send "+total+" BTC").fadeOut().fadeIn();
				thisbtn.attr('disabled',false);
			}
	...
	function walletBalance(){
		...
		coinjs.addressBalance($("#walletAddress").html(),function(data){
			// alert(data)
			var bal = parseInt(data)/100000000
			$("#walletBalance").html(bal+"").attr('rel',bal).fadeOut().fadeIn();
	...
	function scannerStart(){
		...
		if(navigator.getUserMedia){
			...
			QCodeDecoder().decodeFromCamera(document.getElementById('videoReader'), function(er,data){
				if(!er){
					var match = data.match(/^awesome\:([A][a-z0-9]{26,33})/i);
	...
	function listUnspentBlockrio_BitcoinMainnet(redeem){
		...
		$.ajax ({
			type: "POST",
			url: "https://blockchain.coinawesome.com/api/addr/"+redeem.addr+"/utxo?unconfirmed=1",
	...
	$(".qrcodeBtn").click(function(){
		...
		if(ta.length>0){
			...
			qrstr = "awesome:"+$('.address',thisbtn).val();
		}
